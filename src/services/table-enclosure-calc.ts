/**
 * Table and Enclosure Calculator Utilities (migrated from sample code)
 * Updated: 2025-06-08
 * Author: Deej Potter (original), migrated by GitHub Copilot
 * Description: Utility functions for the Table and Enclosure Calculator backend service.
 * These functions handle calculations for different parts of a table and enclosure.
 *
 * NOTE: This file was migrated from sample code/app/table-enclosure-calculator/calcUtils.ts
...
import {
	EXTRUSION_OPTIONS,
	ExtrusionDimensions,
} from "./table-enclosure-constants";
...
// Define interfaces for panel objects to avoid 'never[]' type inference
interface DoorPanel {
	position: string;
	width: number;
	height: number;
	length: number; // For BOM/test compatibility
	thickness: number;
	notes: string;
}

interface MaterialPanel {
	position: string;
	width: number;
	length: number;
	thickness: number;
	height?: number; // Optional, as some panels are defined by width/length (top/bottom)
}
...
export const calculateEnclosureMaterials = (
	...
	// Get extrusion info from constants, providing default fallbacks
	const extrusion2020: ExtrusionDimensions = EXTRUSION_OPTIONS.find(
		(e) => e.id === "20x20-20"
	) || {
		id: "20x20-20",
		name: "20x20 V-Slot Extrusion",
		width: 20,
		height: 20,
		slotDepth: 6,
		slotWidth: 6, // Added default slotWidth
		sku: "LR-2020-S-1",
		length: 1,
		colour: "Silver",
	};

	const extrusion2040: ExtrusionDimensions = EXTRUSION_OPTIONS.find(
		(e) => e.id === "20x40-20"
	) || {
		id: "20x40-20",
		name: "20x40 V-Slot Extrusion",
		width: 20,
		height: 40,
		slotDepth: 6,
		slotWidth: 6, // Added default slotWidth
	...
	// const bottomExtrusionType = "2020"; // Not directly used, but for clarity
	const bottomExtrusionHeight = extrusion2020.height;

	// Vertical extrusion type (always 2020)
	// const verticalExtrusionType = "2020"; // Not directly used, but for clarity
	...
	// The original code had a slight confusion in horizontalLength/Width vs effectiveLength/Width.
	// The `effectiveLength` and `effectiveWidth` correctly calculate the length of the horizontal extrusions
	// based on whether the provided dimensions are internal or external.

	const effectiveLength = isOutsideDimension
		? dimensions.length - verticalExtrusionWidth // If OD, subtract one vertical profile width (since it's a corner)
		: dimensions.length; // If ID, this is the length of the horizontal member
	const effectiveWidth = isOutsideDimension
		? dimensions.width - verticalExtrusionWidth // If OD, subtract one vertical profile width
		: dimensions.width; // If ID, this is the width of the horizontal member
	...
	const effectiveHeight = isOutsideDimension
		? dimensions.height - (topLengthExtrusionHeight + bottomExtrusionHeight) // Assuming topLengthExtrusionHeight is representative for top rail height
		: dimensions.height;
	/**
...
export const calculateDoorMaterials = (
	...
	const extrusion20Series: ExtrusionDimensions = EXTRUSION_OPTIONS.find((e) =>
	...
	const slotDepth = extrusion20Series.slotDepth;
	const extrusionWidth = extrusion20Series.width;
...
export const calculatePanelMaterials = (
	 ...
	 */
	// Get slot depth information from EXTRUSION_OPTIONS
	const extrusionInfo: ExtrusionDimensions = EXTRUSION_OPTIONS.find((e) =>
		e.id.includes("20x20-20")
	) || {
		id: "20x20-20",
		name: "20x20 V-Slot Extrusion",
	...
	const slotDepth = extrusionInfo.slotDepth;
	const extrusionWidth = extrusionInfo.width;
	...
	if (materialConfig.panelConfig.left) {
		panels.push({
			...
			length: panelSideWidth, // For consistency, though height is more descriptive here
			thickness: materialConfig.thickness,
		});
	...
	if (materialConfig.panelConfig.right) {
		panels.push({
			...
			width: panelSideWidth,
			height: panelSideHeight,
			length: panelSideWidth, // For consistency
			thickness: materialConfig.thickness,
		});
		totalArea += panelSideWidth * panelSideHeight;
	}
	if (materialConfig.panelConfig.back) {
		panels.push({
			...
			length: panelFrontBackWidth, // For consistency
			thickness: materialConfig.thickness,
		});
...
// Export constants for testing or if needed by other services
